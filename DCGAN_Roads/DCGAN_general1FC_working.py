...
with tf.variable_scope("Generator"):
    ...
    g_conv2 = tf.Variable(tf.truncated_normal([8, 8, 64, 128], stddev = 0.02))
    g_conv2_bias = tf.constant(0.1, shape=[64])

    g_conv3 = tf.Variable(tf.truncated_normal([16, 16, 3, 64], stddev = 0.02))
    g_conv3_bias = tf.constant(0.1, shape=[3])
...
def Generator(z):
    ...
    with tf.variable_scope("G_added"):
        ...
        g_added = tf.nn.conv2d_transpose(g2, g_conv_added, [batch_size, 8, 8, 128], [1, 2, 2, 1], padding="SAME")
    ...
    with tf.variable_scope("G3"):
        ...
        g3 = tf.nn.conv2d_transpose(g_added, g_conv2, [batch_size, 16, 16, 64], [1, 2, 2, 1], padding="SAME")
    ...
    with tf.variable_scope("G4"):
        ...
        g4 = tf.nn.conv2d_transpose(g3, g_conv3, [batch_size, 32, 32, 3], [1, 2, 2, 1], padding="SAME")
...
with tf.variable_scope("Discriminator"):
    ...
    d_conv1 = tf.Variable(tf.truncated_normal([5, 5, 3, 16], stddev = 0.02))
...
def Discriminator(x):
    ...
    with tf.variable_scope("D1"):
        ...
        d1 = tf.nn.conv2d(x, d_conv1, strides=[1, 2, 2, 1], padding="SAME") + d_bias1
...
if not os.path.exists('generated_samples_general_1FCtanh/'):
    os.makedirs('generated_samples_general_1FCtanh/')

it = 0

iterator=1
labels = open("labels_full/train_labels.list")
train_examples_list = []
for line in labels:
    if int(line) == 1:
        temp_image = misc.imread("cropped_data/{:06}.png".format(iterator))
        train_examples_list.append(temp_image)
    iterator+=1

train_examples_array = np.asarray(train_examples_list) / 255

iter_in_epoch = train_examples_array.shape[0] // batch_size
...
for i in range(num_epochs):
    if i % 100 == 0:
        ...
        fig = plot(samples)
        plt.savefig
        plt.savefig('generated_samples_general_1FCtanh/{}.png'.format(str(it).zfill(3)), bbox_inches='tight')
        it += 1
        plt.close(fig)

    for temp_iter in range(iter_in_epoch):

        x_samples = train_examples_array[temp_iter*batch_size:(temp_iter+1)*batch_size]
...
saver.save(sess, "weightstanh/model.ckpt")
