...
if not os.path.exists('generated_samples1FC/'):
    os.makedirs('generated_samples1FC/')
...
for i in range(num_iters):
    if i % 5000 == 0:
        ...
        fig = plot(samples)
        plt.savefig
        plt.savefig('generated_samples1FC/{}.png'.format(str(it).zfill(3)), bbox_inches='tight')
        it += 1
        plt.close(fig)
