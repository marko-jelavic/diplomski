...
generator.add(layers.Dense(1024, input_shape=(100,)))
generator.add(tf.keras.layers.BatchNormalization())
generator.add(layers.ReLU())
generator.add(layers.Dense(2048))
generator.add(tf.keras.layers.BatchNormalization())
generator.add(layers.ReLU())
generator.add(layers.Dense(6272))
generator.add(tf.keras.layers.BatchNormalization())
generator.add(layers.ReLU())
...
def train(ds):
    ...
    for step in range(NUM_STEPS):
        ...
        if step % 5000 == 0:
            ...
            plt.savefig('new_samples3FC/{}.png'.format(str(it).zfill(3)), bbox_inches='tight')
            plt.close(fig)
            it += 1
