...
with tf.variable_scope("Generator"):
    ...
    g_w_added = tf.Variable(tf.truncated_normal([1024, 2048], stddev = 0.02))
    g_b_added = tf.constant(0.1, shape=[2048])
...
def Generator(z):
    ...
    with tf.variable_scope("G_added"):
        ...
        g_added = tf.matmul(g1, g_w_added) + g_b_added
...
if not os.path.exists('generated_samples3FC/'):
    os.makedirs('generated_samples3FC/')
...
for i in range(num_iters):
    if i % 5000 == 0:
        ...
        fig = plot(samples)
        plt.savefig
        plt.savefig('generated_samples3FC/{}.png'.format(str(it).zfill(3)), bbox_inches='tight')
        it += 1
        plt.close(fig)
