...
generator.add(layers.Dense(2048, activation=None, input_shape=(100,),
...
generator.add(layers.Conv2DTranspose(filters=256, kernel_size=(2, 2), strides=(2, 2), padding="same", activation=None,
                                     kernel_initializer=tf.keras.initializers.truncated_normal(stddev=0.02),
                                     bias_initializer=tf.keras.initializers.Constant(0.1)))
generator.add(tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=0.00001, scale=True))
generator.add(layers.ReLU())
generator.add(layers.Conv2DTranspose(filters=128, kernel_size=(4, 4), strides=(2, 2), padding="same", activation=None,
                                     kernel_initializer=tf.keras.initializers.truncated_normal(stddev=0.02),
                                     bias_initializer=tf.keras.initializers.Constant(0.1), output_padding=(1, 1)))
generator.add(tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=0.00001, scale=True))
generator.add(layers.ReLU())
generator.add(layers.Conv2DTranspose(filters=64, kernel_size=(7, 7), strides=(2, 2), padding="same", activation=None,
                                     kernel_initializer=tf.keras.initializers.truncated_normal(stddev=0.02),
                                     bias_initializer=tf.keras.initializers.Constant(0.1)))
generator.add(tf.keras.layers.BatchNormalization(momentum=0.9, epsilon=0.00001, scale=True))
generator.add(layers.ReLU())
...
g_optim = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5)
d_optim = tf.keras.optimizers.Adam(learning_rate=0.0001, beta_1=0.5)
...
def train(ds):
    ...
    for step in range(NUM_STEPS):
        ...
        if step % 5000 == 0:
            ...
            plt.savefig('new_samples4/{}.png'.format(str(it).zfill(3)), bbox_inches='tight')
            plt.close(fig)
            it += 1
